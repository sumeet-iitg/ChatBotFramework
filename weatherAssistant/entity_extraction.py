...
def get_location_entity(text):
    s = SennaSRLTagger("C:/Users/Sumeet Singh/Documents/Lectures/11-754 DialogueSystems/senna_ner/senna")
    entities = s.tag(text.split())
    location_split = []
    for index, tokens in entities:
        if "LOC" in tokens[1]:
            location_split.append(tokens[0])
    location = " ".join(location_split)
    return location

#print(get_location_entity("What\'s the weather in Pittsburgh like?"))
# print(s.tag("""What's the weather in Pittsburgh like?""".split()))